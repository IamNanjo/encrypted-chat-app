...
async function parseChat(rawChat: RawChat): Promise<Chat> {
    ...
    for (let i = 0, iLen = rawChat.members.length; i < iLen; i++) {
        ...
        for (const device of member.devices) {
            const parsedKey = await importKey(
...
<template>
    <aside :class="isOpen ? 'chat-menu open' : 'chat-menu'">
        ...
        <div class="chat-list">
            <TransitionGroup name="chat-list-transition">
                <div class="chat-list__empty">No chats found</div>
                <button
                    v-for="chat in chats"
                    :key="chat.id"
